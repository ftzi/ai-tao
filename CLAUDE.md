...
**Workflow Rule:** **MUST ALWAYS** run `bun ok` after finishing a task or when facing issues. This command runs type checking and linting across the entire codebase and **MUST** fully pass before considering a task complete.
...
This document uses special keywords to indicate the severity of requirements:

- **MUST** / **ALWAYS** - Mandatory requirements that must be followed without exception. These are critical rules that affect code quality, workflow correctness, or project integrity. Violations will cause issues.

- **NEVER** - Prohibited actions that must be avoided. These represent patterns or practices that will cause problems, break functionality, or violate project standards.

When you see these keywords, pay extra attention - they represent hard-learned lessons and project-critical requirements.
...
**NEVER run `bun dev` or `next dev` directly.** The dev server causes lock file issues and port conflicts. Instead:
...
- **MUST ALWAYS use `bun ok`** for type checking and linting - **NEVER** use `bun ts`, `bun lint`, or `tsc` directly
- **NEVER run `tsc` directly** - not even for single files - **MUST ALWAYS** use `bun ok`
- **NEVER run `bun build` or `bun run build`** - only use `bun ok` for verification
- **`bun ok` MUST ALWAYS be run from the project root directory**
  - **NEVER** run it from subdirectories like `apps/example` or `packages/*`
  - **MUST ALWAYS** navigate to the root first: `cd /Users/ftzi/dev/nextbook && bun ok`
  - This is a Turborepo monorepo - the command **MUST** run from root to check all packages
- `bun ok` runs both type checking and linting, leverages Turbo cache, and **MUST ALWAYS** be used
...
- **NEVER use `interface`** - **MUST ALWAYS** use `type` instead
...
- **MUST ALWAYS clean up debug code** - Remove all console logs and debugging code once the root cause is found

**React Conventions:**

- **MUST ALWAYS follow the Rules of Hooks**:
  - **MUST** only call hooks at the top level - **NEVER** inside loops, conditions, or nested functions
  - **NEVER** return early if there's a hook later in the component
  - Hooks **MUST** be called in the same order every render

**Testing:**

- **Unit tests are REQUIRED** - **MUST ALWAYS** add unit tests when adding or modifying functions/utilities. Tests ensure a solid and reliable product.
- Test files **MUST** be co-located with source files (e.g., `schema.ts` → `schema.test.ts`)
...
- **Post-task test verification** - After completing any task, **MUST** verify test coverage for changed files:
...
- When making ANY changes to the nextbook package:
  1. Update the code
  2. **MUST immediately update the root README.md to reflect those changes**
  3. **MUST** verify code examples in README are copy-paste correct
- The root README is the primary documentation for users
- **Manual Setup MUST match CLI output** - The manual setup steps in README **MUST** produce the same result as `npx nextbook init`. When updating CLI templates (`src/cli/templates.ts`), **MUST** also update the README manual setup section to match.
...
- When making changes to nextbook features or API:
  ...
  3. **MUST update apps/web if affected** (code examples, feature descriptions, comparison table)
...
- **Package Manager:** **MUST ALWAYS** use `bun` instead of npm/yarn/pnpm
